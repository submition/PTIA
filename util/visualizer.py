...
class Visualizer():
    def __init__(self, opt):
        self.opt = opt
        self.save_dir = join(opt['path']['experiments_root'], 'visual')
        if opt['is_train']:
            self.name = opt['name']
            self.writer = SummaryWriter(logdir=join(self.save_dir, 'log'))
        else:
            self.name = opt['name']
            self.save_dir = join(opt['path']['experiments_root'], 'visual', 'test_log')
    ...
    def print_current_losses(self, epoch, iters, losses,
                             t_comp, total_iters, lr):
        message = '(epoch: %d, iters: %d, time: %.3f, lr: %.4f) ' \
                  % (epoch, iters, t_comp, lr)
    ...
    def print_psnr(self, epoch, total_epoch, time_val, mean_psnr, print_psnr=True):
        ...
        if print_psnr:
            print('End of iter %d / %d (Val) \t Time Taken: %.3f s \t PSNR: %f'
