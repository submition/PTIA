...
class VideoCompressRecurrentModel(VideoBaseModel):

    def __init__(self, opt):
        super(VideoCompressRecurrentModel, self).__init__(opt)
        if self.is_train:
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for i in range(rank, num_folders + num_pad, world_size):
            ...
            start_idx, max_idx = val_data['idx'].split('/')
            # print("%%%%%%%%%%%%%%%", start_idx, max_idx)
            num_frame = val_data['num_frame']
                    ...
                    if save_img:
                        if self.opt['is_train']:
                            ...
                            if self.center_frame_only:  # vimeo-90k
                                ...
                                frame_idx = int(start_idx) * num_frame + idx
                                print("$$$$$$$$$$$$$$$$$$$$$$$$", frame_idx)
                                img_path = osp.join(self.opt['path']['visualization'], dataset_name, folder,
                                                    f"{frame_idx:08d}_{self.opt['name']}.png")
