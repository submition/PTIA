...
class Vimeo90KCompressDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(Vimeo90KCompressDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])

        # self.meta_root = Path(opt['meta_info_file'])
        self.lq_subroot = opt['lq_subroot']
        # self.meta_subroot = opt['meta_subroot']
...
@DATASET_REGISTRY.register()
class Vimeo90KCompressRecurrentDataset(Vimeo90KCompressDataset):

    def __init__(self, opt):
        super(Vimeo90KCompressRecurrentDataset, self).__init__(opt)
    ...
    def __getitem__(self, index):
        ...
        clip, seq = key.split('/')   # key example: 00001/0001
        # number, clip, seq = key.split('/')  # key example: 00001/0001
        ...
        for neighbor in self.neighbor_list:
            if self.is_lmdb:
                ...
                img_lq_path = self.lq_root / self.lq_subroot[sub_id] / clip / seq / f'im{neighbor}.png'
        ...
        return {'lq': img_lqs, 'gt': img_gts, 'key': key,
                 'crf_rate':self.lq_subroot[sub_id]}
...
def get_meta(meta_pth):
    ...
    for mv in mvs:
        ...
        if isinstance(framenum, str):
           framenum, source, blockx, blocky, srcx, srcy, dstx, dsty, scale, frame_type = int(framenum), int(source), int(blockx), int(blocky), int(srcx),\
                                                                                      int(srcy), int(dstx), int(dsty), int(scale), int(frame_type)
        framenum -= 1
