...
# try:
from basicsr.pwc.correlation import correlation # the custom cost volume layer
# except:
...
class PWCNET(torch.nn.Module):
	def __init__(self):
		...
		# 					   in torch.load('/home/jq/Real/EAVSR-main/pwc/pwc-default.pytorch', map_location=lambda storage,
		# 					   loc: storage, pickle_module=pickle).items() })

		# self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight
		#     in torch.hub.load_state_dict_from_url(url='http://content.sniklaus.com/github/pytorch-pwc/network-' +
		#     'default' + '.pytorch', file_name='pwc-' + 'default').items()})
...
# 	pickle.load = partial(pickle.load, encoding="latin1")
# 	pickle.Unpickler = partial(pickle.Unpickler, encoding="latin1")
# 	state_dict = torch.load('/home/jq/Real/EAVSR-main/pwc/pwc-default.pytorch', map_location=lambda storage,loc: storage,pickle_module=pickle)
